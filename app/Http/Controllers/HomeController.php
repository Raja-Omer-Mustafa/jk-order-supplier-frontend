...
class HomeController extends Controller
{
    ...
    public function getblog()
    {
        ...
        $result = $this->post('getblog', ['data' => ['get_blogs' => 1, 'limit' => '150']]);

        $this->html('blog', ['data' => $result]);
    }
    ...
    public function getblogsingle($slug)
    {
        ...
        if (count($blogsingle))
        {
            ...
            $this->html('blogsingle', ['data' => $blogsingle, 'datarecentblog' => $resultrecentblog, 'blogcategories' => $blogcategories]);
    ...
    {
        $data = $this->post('getblog', ['data' => ['get_blogs' => 1, 'limit' => '150', 'category_slug' => $slug]]);

        $this->html('blogcategory', ['data' => $data]);
    ...
    public function flash_sale($slug)
    {
        ...
        //dd($result);
        $this->html('flash-sales', ['data' => $result]);
    }
    ...
    public function product($slug)
    {
        ...
        $rating = $this->post('products', ['data' => ['get_product_reviews' => 1, 'slug' => $slug]]);
        //dd($rating);
    ...
    public function category(Request $request, $slug)
    {
        ...
        if (Cache::has('categories') && is_array(Cache::get('categories')) && isset(Cache::get('categories') [$slug]))
        {
            ...
            if (isset($subCategory['error']) && $subCategory['error'])
            {
                ...
                $limit = 200;
                $number_of_pages = $total / $limit;
                $this->html('sub-categories', ['title' => $category->name, 'breadcrumb' => $breadcrumb, 'category' => $category, 'list' => $list, 'limit' => $limit, 'number_of_pages' => $number_of_pages]);
            }
            else
            {
                ...
                $limit = 200;
                $number_of_pages = $total / $limit;
                $this->html('sub-categories', ['title' => $category->name, 'breadcrumb' => $breadcrumb, 'category' => $category, 'sub-categories' => $subCategory, 'list' => $list, 'limit' => $limit, 'number_of_pages' => $number_of_pages]);
            }
        }
        else
        {
